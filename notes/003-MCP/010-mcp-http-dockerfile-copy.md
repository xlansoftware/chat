---
title: MCP-HTTP Dockerfile - copy
---
--- message ---
role: user
---
I am developing a nextjs app using assistant-ui. I want to provide MCP server functionality to my app using:

```typescript
import { createMCPClient } from '@ai-sdk/mcp';
import { StreamableHTTPClientTransport } from '@modelcontextprotocol/sdk/client/streamableHttp.js';

const url = new URL('https://your-server.com/mcp');
const mcpClient = await createMCPClient({
  transport: new StreamableHTTPClientTransport(url, {
    sessionId: 'session_123',
  }),
});
```
I want to use the servers at modelcontextprotocol/servers, however they only support stdio as transport.

```json
{
  "mcpServers": {
    "filesystem": {
      "command": "docker",
      "args": [
        "run",
        "-i",
        "--rm",
        "--mount", "type=bind,src=/Users/username/Desktop,dst=/projects/Desktop",
        "--mount", "type=bind,src=/path/to/other/allowed/dir,dst=/projects/other/allowed/dir,ro",
        "--mount", "type=bind,src=/path/to/file.txt,dst=/projects/path/to/file.txt",
        "mcp/filesystem",
        "/projects"
      ]
    }
  }
}
```

Are there ready available MCP-over-HTTP gateway projects that I can use so I can expose the stdio servers via http?

the whole app will run with docker compose. the MCP server will run as separate service, available only to the nextjs app service backend, so the security is not a concern. Please, create thin http wrapper using MCP SDK. let it be a nodejs project and take the mcpServers json configuration for the MCP server to start.

--- message ---
role: assistant
---
Below is a thin “stdio MCP server → MCP Streamable HTTP” gateway as a standalone Node.js (TypeScript) service. It:

- Reads an `mcpServers` JSON config (same shape as your example)
- Spawns the selected server command (often `docker run ...`)
- Bridges **Streamable HTTP (server transport)** ⇄ **stdio (client transport)** by forwarding MCP JSON-RPC messages
- Intended to run inside docker-compose, reachable only from your Next.js backend

## Project layout

```
mcp-http-gateway/
  package.json
  tsconfig.json
  src/
    index.ts
```

## 1) package.json

```json
{
  "name": "mcp-http-gateway",
  "private": true,
  "type": "module",
  "scripts": {
    "dev": "node --enable-source-maps --loader ts-node/esm src/index.ts",
    "build": "tsc -p tsconfig.json",
    "start": "node dist/index.js"
  },
  "dependencies": {
    "@modelcontextprotocol/sdk": "^1.0.0",
    "express": "^4.19.2"
  },
  "devDependencies": {
    "ts-node": "^10.9.2",
    "typescript": "^5.5.4"
  }
}
```

> If you’re already using `@ai-sdk/mcp`, you don’t need it in this gateway; this gateway is just an MCP transport bridge.

## 2) tsconfig.json

```json
{
  "compilerOptions": {
    "target": "ES2022",
    "module": "ES2022",
    "moduleResolution": "Bundler",
    "outDir": "dist",
    "strict": true,
    "esModuleInterop": true,
    "skipLibCheck": true
  },
  "include": ["src"]
}
```

## 3) src/index.ts

```ts
import http from "node:http";
import { Readable } from "node:stream";
import fs from "node:fs";

import { StreamableHTTPServerTransport } from "@modelcontextprotocol/sdk/server/streamableHttp.js";
import { StdioClientTransport } from "@modelcontextprotocol/sdk/client/stdio.js";

type MCPServersConfig = {
  mcpServers: Record<
    string,
    {
      command: string;
      args?: string[];
      env?: Record<string, string>;
      cwd?: string;
      stderr?: "inherit" | "pipe" | "ignore";
    }
  >;
};

function loadConfig(): MCPServersConfig {
  const configPath = process.env.MCP_SERVERS_CONFIG ?? "/config/mcpServers.json";
  return JSON.parse(fs.readFileSync(configPath, "utf8"));
}

function pickServer(cfg: MCPServersConfig) {
  const name = process.env.MCP_SERVER_NAME ?? Object.keys(cfg.mcpServers)[0];
  if (!name) throw new Error("No mcpServers configured");

  const server = cfg.mcpServers[name];
  if (!server) throw new Error(`MCP server "${name}" not found in config`);
  return { name, server };
}

/**
 * Minimal bidirectional message bridge between two MCP transports.
 * Both transports must have:
 *   - onmessage: (msg) => void
 *   - send(msg): Promise<void> | void
 */
function bridgeTransports(a: any, b: any) {
  a.onmessage = async (msg: unknown) => {
    await b.send(msg);
  };
  b.onmessage = async (msg: unknown) => {
    await a.send(msg);
  };
}

/**
 * Convert Node IncomingMessage -> Web Request
 * (Node 18+ provides global Request/Headers/Response)
 */
async function nodeToWebRequest(req: http.IncomingMessage): Promise<Request> {
  const host = req.headers.host ?? "localhost";
  const url = new URL(req.url ?? "/", `http://${host}`);

  const headers = new Headers();
  for (const [k, v] of Object.entries(req.headers)) {
    if (Array.isArray(v)) headers.set(k, v.join(", "));
    else if (v != null) headers.set(k, v);
  }

  // For GET/HEAD, body must be undefined
  const method = req.method ?? "GET";
  const body =
    method === "GET" || method === "HEAD"
      ? undefined
      : (Readable.toWeb(req) as any);

  return new Request(url, {
    method,
    headers,
    body,
    // Required when passing a stream body in some runtimes
    // @ts-expect-error duplex exists in Node's fetch
    duplex: body ? "half" : undefined,
  });
}

async function main() {
  const port = Number(process.env.PORT ?? 8787);

  const server = http.createServer(async (req, res) => {
    // Only handle MCP endpoint
    if (!req.url?.startsWith("/mcp")) {
      res.statusCode = 404;
      res.end("Not found");
      return;
    }

    const cfg = loadConfig();
    const { name, server: mcpSrv } = pickServer(cfg);

    // Create a fresh stdio server process per HTTP session/request.
    // (Simple + avoids cross-session interference.)
    const stdioTransport = new StdioClientTransport({
      command: mcpSrv.command,
      args: mcpSrv.args ?? [],
      env: { ...(mcpSrv.env ?? {}) },
      // env: { ...process.env, ...(mcpSrv.env ?? {}) },
      cwd: mcpSrv.cwd,
      stderr: mcpSrv.stderr ?? "inherit",
    });

    // One HTTP transport instance for this request handler.
    // NOTE: If you want real session persistence across multiple HTTP requests,
    // you would typically keep ONE StreamableHTTPServerTransport instance globally
    // and call handleRequest() for each request. This minimal version stays stateless.
    const httpTransport = new StreamableHTTPServerTransport({
      // set true if you want non-SSE JSON responses only
      enableJsonResponse: false,
    });

    try {
      await stdioTransport.start();

      // Bridge before handling the HTTP request so any inbound messages get forwarded
      bridgeTransports(httpTransport as any, stdioTransport as any);

      const webReq = await nodeToWebRequest(req);

      await httpTransport.handleRequest(req, res);

      // Ensure cleanup when the connection drops
      res.on("close", async () => {
        try {
          await stdioTransport.close();
        } catch { }
        try {
          await httpTransport.close();
        } catch { }
      });

      console.log(`[gateway] ${req.method} ${req.url} -> stdio "${name}"`);
    } catch (err: any) {
      try {
        await stdioTransport.close();
      } catch { }
      try {
        await httpTransport.close();
      } catch { }

      res.statusCode = 500;
      res.end(err?.stack ?? err?.message ?? "Gateway error");
    }
  });

  server.listen(port, () => {
    console.log(`[gateway] listening on :${port}`);
  });
}

main().catch((e) => {
  console.error(e);
  process.exit(1);
});

```

## Next.js usage (your side)
Point your client at the gateway service:

```ts
import { createMCPClient } from "@ai-sdk/mcp";
import { StreamableHTTPClientTransport } from "@modelcontextprotocol/sdk/client/streamableHttp.js";

const url = new URL("http://mcp-gateway:8787/mcp"); // docker compose service name
const mcpClient = await createMCPClient({
  transport: new StreamableHTTPClientTransport(url, { sessionId: "session_123" }),
});
```

### Important note about “sessions”
The above implementation is **stateless** in the sense that each HTTP request spawns a new stdio server. If your client uses `sessionId` and expects the server to maintain a session across multiple HTTP calls (typical with Streamable HTTP), you probably want:

- **one global** `StreamableHTTPServerTransport({ sessionIdGenerator, onsessioninitialized, onsessionclosed })`
- a `Map<sessionId, StdioClientTransport>` so each session gets its own stdio server
- `serverTransport.handleRequest(req)` on every request, while keeping stdio transports alive until `DELETE` closes the session

If you want that (it’s usually what you want with `StreamableHTTPClientTransport`), tell me whether you want **one stdio process per sessionId** or **one shared stdio process**, and I’ll provide the session-aware version using `onsessioninitialized/onsessionclosed`.

--- message ---
role: user
---
please, implement the session-aware version with one shared stdio process

--- message ---
role: assistant
---
Below is a **session-aware** MCP-over-HTTP gateway that keeps **one shared stdio MCP server process** for all HTTP sessions. It uses a single global `StreamableHTTPServerTransport` and a single global `StdioClientTransport`, and bridges messages between them.

Important caveat: many MCP stdio servers implicitly assume **one client / one conversation state**. Sharing one stdio process across multiple HTTP sessions can cause cross-session state leakage or race/interleaving issues if multiple clients use it concurrently. But this matches what you asked for.

## `src/index.ts` (session-aware, shared stdio)

```ts
import http from "node:http";
import fs from "node:fs";

import { StreamableHTTPServerTransport } from "@modelcontextprotocol/sdk/server/streamableHttp.js";
import { StdioClientTransport } from "@modelcontextprotocol/sdk/client/stdio.js";

type MCPServersConfig = {
  mcpServers: Record<
    string,
    {
      command: string;
      args?: string[];
      env?: Record<string, string>;
      cwd?: string;
      stderr?: "inherit" | "pipe" | "ignore";
    }
  >;
};

function loadConfig(): MCPServersConfig {
  const configPath = process.env.MCP_SERVERS_CONFIG ?? "/config/mcpServers.json";
  return JSON.parse(fs.readFileSync(configPath, "utf8"));
}

function pickServer(cfg: MCPServersConfig) {
  const name = process.env.MCP_SERVER_NAME ?? Object.keys(cfg.mcpServers)[0];
  if (!name) throw new Error("No mcpServers configured");

  const server = cfg.mcpServers[name];
  if (!server) throw new Error(`MCP server "${name}" not found in config`);
  return { name, server };
}

/**
 * Bridge MCP messages between transports:
 * - HTTP -> stdio
 * - stdio -> HTTP
 *
 * Assumes both transports expose:
 *   - onmessage = (msg) => void
 *   - send(msg)
 */
function bridgeTransports(httpTransport: any, stdioTransport: any) {
  httpTransport.onmessage = async (msg: unknown) => {
    await stdioTransport.send(msg);
  };

  stdioTransport.onmessage = async (msg: unknown) => {
    await httpTransport.send(msg);
  };
}

async function main() {
  const port = Number(process.env.PORT ?? 8787);

  // Load config + start ONE shared stdio MCP server process
  const cfg = loadConfig();
  const { name, server: mcpSrv } = pickServer(cfg);

  const stdioTransport = new StdioClientTransport({
    command: mcpSrv.command,
    args: mcpSrv.args ?? [],
    env: { ...(mcpSrv.env ?? {}) },
    // If you need full env inheritance, use:
    // env: { ...process.env, ...(mcpSrv.env ?? {}) },
    cwd: mcpSrv.cwd,
    stderr: mcpSrv.stderr ?? "inherit",
  });

  // ONE shared Streamable HTTP transport handling multiple sessions.
  // It will manage sessions via headers/cookies/etc. (handled internally by SDK).
  const httpTransport = new StreamableHTTPServerTransport({
    enableJsonResponse: false, // keep streaming/SSE behavior
  });

  // Start shared stdio process and connect bridge
  await stdioTransport.start();
  bridgeTransports(httpTransport as any, stdioTransport as any);

  // Clean shutdown
  const shutdown = async () => {
    try {
      await httpTransport.close();
    } catch {}
    try {
      await stdioTransport.close();
    } catch {}
    process.exit(0);
  };
  process.on("SIGINT", shutdown);
  process.on("SIGTERM", shutdown);

  // Optional: if the stdio process dies, crash the gateway (so compose restarts it)
  // (If you want auto-restart inside the process, say so and I’ll adjust.)
  try {
    // @ts-expect-error transport impl detail; not always present
    stdioTransport.process?.once?.("exit", (code: number) => {
      console.error(`[gateway] stdio server exited with code=${code}; exiting`);
      process.exit(1);
    });
  } catch {}

  const server = http.createServer(async (req, res) => {
    try {
      if (!req.url?.startsWith("/mcp")) {
        res.statusCode = 404;
        res.end("Not found");
        return;
      }

      // Let the SDK handle session-aware Streamable HTTP requests
      await httpTransport.handleRequest(req, res);

      // NOTE: Do NOT close transports here; they are shared/globally managed.
    } catch (err: any) {
      res.statusCode = 500;
      res.end(err?.stack ?? err?.message ?? "Gateway error");
    }
  });

  server.listen(port, () => {
    console.log(`[gateway] listening on :${port}`);
    console.log(`[gateway] sharing ONE stdio MCP server: "${name}" (${mcpSrv.command} ${mcpSrv.args?.join(" ") ?? ""})`);
  });
}

main().catch((e) => {
  console.error(e);
  process.exit(1);
});
```

## Config file example (`/config/mcpServers.json`)

Same as your shape:

```json
{
  "mcpServers": {
    "filesystem": {
      "command": "docker",
      "args": [
        "run",
        "-i",
        "--rm",
        "--mount", "type=bind,src=/Users/username/Desktop,dst=/projects/Desktop",
        "mcp/filesystem",
        "/projects"
      ]
    }
  }
}
```

## Next.js client usage

```ts
import { createMCPClient } from "@ai-sdk/mcp";
import { StreamableHTTPClientTransport } from "@modelcontextprotocol/sdk/client/streamableHttp.js";

const url = new URL("http://mcp-gateway:8787/mcp");
const mcpClient = await createMCPClient({
  transport: new StreamableHTTPClientTransport(url, { sessionId: "session_123" }),
});
```


If you tell me which `modelcontextprotocol/servers` image(s) you’re targeting (filesystem, git, sqlite, etc.), I can also provide a working `docker-compose.yml` showing the gateway service + correct Docker socket / mounts setup (common gotcha: the gateway container needs access to `/var/run/docker.sock` if it runs `docker run ...

--- message ---
role: user
---
create a Dockerfile for the mcp-gateway. let it run as non-root user. I do not need docker ci. I need to be able to use npx

--- message ---
role: assistant
---
```dockerfile
# syntax=docker/dockerfile:1.6

################
# build stage  #
################
FROM node:20-bookworm-slim AS build
WORKDIR /app

COPY package.json package-lock.json* ./
RUN --mount=type=cache,target=/root/.npm \
    npm install

COPY tsconfig.json ./
COPY src ./src
RUN npm run build


#####################
# runtime (non-root)#
#####################
FROM node:20-bookworm-slim AS runtime
WORKDIR /app

ENV NODE_ENV=production
ENV PORT=8787

# Copy build output + node_modules so `npx` can run project-local CLIs.
# (npm is included with the Node image; npx is part of npm.)
COPY --from=build /app/package.json ./package.json
COPY --from=build /app/package-lock.json* ./  # optional
COPY --from=build /app/node_modules ./node_modules
COPY --from=build /app/dist ./dist

# Create and use a non-root user
RUN useradd -m -u 10001 -s /usr/sbin/nologin appuser \
  && chown -R appuser:appuser /app

USER appuser

EXPOSE 8787
CMD ["node", "dist/index.js"]
```

Notes:
- This keeps **npm/npx available** (they ship with the official Node image). You can run `npx ...` inside the container.
- If your `mcpServers.json` uses `"command": "npx"` (or a local bin), it will work as long as:
  - the package is in `dependencies` (so it exists in `node_modules/.bin`), or
  - you allow `npx` to download it at runtime (requires network + writable npm cache).
- If you plan to use runtime `npx` downloads as non-root, set a writable cache dir, e.g. in compose:
  - `environment: { NPM_CONFIG_CACHE: /tmp/.npm }` or mount a volume to `/home/appuser/.npm`.

--- message ---
role: user
---
please, verify my docker-compose.yaml

```
services:
  nextjs:
    container_name: nextjs-app
    build:
      context: ../packages/folderprototype
      dockerfile: Dockerfile
    # image: nextjs-app:latest
    read_only: true
    tmpfs:
      - /tmp
    cap_drop:
      - ALL
    security_opt:
      - no-new-privileges:true
    ports:
      - "${PORT:-3000}:${PORT:-3000}"
    environment:
      MCP_SERVER_ENDPOINT: http://mcp-server:8787/mcp
      STORAGE_TYPE: filesystem
      DATA_STORE_PATH: /data
      CONFIG_PATH: /config
      OPENAI_API_KEY: ${OPENAI_API_KEY:?OPENAI_API_KEY is required}
      PORT: ${PORT:-3000}
      NODE_ENV: production
      LOG_LEVEL: debug
    volumes:
      - ./models.yaml:/config/models.yaml:ro
      - ./preferences.yaml:/config/preferences.yaml
      - ./data:/data
    restart: unless-stopped
  mcp-server:
    container_name: mcp-server
    build:
      context: ../packages/mcp-http-gateway
      dockerfile: Dockerfile
    # image: mcp-server:latest
    environment:
      NODE_ENV: production
    volumes:
      - ./mcp.json:/config/mcpServers.json:ro
    restart: unless-stopped
```

my mcp.json is:

```json
{
  "mcpServers": {
    "everything": {
      "command": "npx",
      "args": [
        "-y",
        "@modelcontextprotocol/server-everything"
      ]
    }
  }
}
```

--- message ---
role: assistant
---
Your compose file is close. The main things to fix/consider:

1) **`nextjs` uses `read_only: true` but you mount writable volumes** (`./preferences.yaml`, `./data:/data`). That’s fine (bind mounts remain writable), but make sure your Next app doesn’t try to write anywhere else (e.g. `.next/cache`, logs, etc.). You already added `tmpfs: /tmp`, which helps.

2) **`mcp-server` probably needs write access for `npx` cache** (because you’re running `npx -y ...` at runtime). Running as non-root, `npx` will try to use a cache directory (typically under the user’s home). In my Dockerfile we created `appuser` with home `/home/appuser`, but we did not mount anything there. It *may* still work (it can write to its home inside the container FS), but if you later set `read_only: true` for `mcp-server` (recommended), you must provide a writable cache (tmpfs or volume) and point `NPM_CONFIG_CACHE` there.

3) **You didn’t set `MCP_SERVERS_CONFIG` / `MCP_SERVER_NAME`** for the gateway. Defaults in the code I gave:
   - config path default: `/config/mcpServers.json` (you mounted exactly that) ✅
   - server name default: first key in `mcpServers` (that will be `"everything"`) ✅  
   So it will work as-is.

4) **Networking:** Compose provides a default network; `http://mcp-server:8787/mcp` resolves by service name ✅. No need to expose ports for `mcp-server` unless you want host access.

5) **`PORT` mapping on nextjs:** `ports: "${PORT:-3000}:${PORT:-3000}"` is okay if your container listens on the same `PORT` value. If your Next Dockerfile always listens on 3000, simpler is `"3000:3000"`.

---

## Recommended adjusted `docker-compose.yaml`

I’d keep `mcp-server` non-root and add a writable npm cache + (optionally) make it read-only too.

```yaml
services:
  nextjs:
    container_name: nextjs-app
    build:
      context: ../packages/folderprototype
      dockerfile: Dockerfile
    read_only: true
    tmpfs:
      - /tmp
    cap_drop:
      - ALL
    security_opt:
      - no-new-privileges:true
    ports:
      - "${PORT:-3000}:${PORT:-3000}"
    environment:
      MCP_SERVER_ENDPOINT: http://mcp-server:8787/mcp
      STORAGE_TYPE: filesystem
      DATA_STORE_PATH: /data
      CONFIG_PATH: /config
      OPENAI_API_KEY: ${OPENAI_API_KEY:?OPENAI_API_KEY is required}
      PORT: ${PORT:-3000}
      NODE_ENV: production
      LOG_LEVEL: debug
    volumes:
      - ./models.yaml:/config/models.yaml:ro
      - ./preferences.yaml:/config/preferences.yaml
      - ./data:/data
    restart: unless-stopped

  mcp-server:
    container_name: mcp-server
    build:
      context: ../packages/mcp-http-gateway
      dockerfile: Dockerfile
    # Recommended hardening (optional but consistent with nextjs)
    read_only: true
    tmpfs:
      - /tmp
    environment:
      NODE_ENV: production
      # Make npx cache writable even with read_only root FS
      NPM_CONFIG_CACHE: /tmp/.npm
      # optional (defaults already match your mount)
      MCP_SERVERS_CONFIG: /config/mcpServers.json
      MCP_SERVER_NAME: everything
    volumes:
      - ./mcp.json:/config/mcpServers.json:ro
    restart: unless-stopped
```

If you don’t want `read_only: true` on `mcp-server`, you can omit it and also omit the npm cache env/tmpfs (but adding them doesn’t hurt).

---

## Your `mcp.json`

This is fine:

```json
{
  "mcpServers": {
    "everything": {
      "command": "npx",
      "args": ["-y", "@modelcontextprotocol/server-everything"]
    }
  }
}
```

Two practical notes:

- `npx -y @modelcontextprotocol/server-everything` will download packages at runtime. For reproducibility and startup speed, consider baking it into the image later (add it to `dependencies` and call the binary directly), but it’s OK for now.
- If the “everything” server uses SSE/stdio correctly, the gateway should bridge it.

If you paste your Next.js Dockerfile, I can sanity-check whether `read_only: true` will break Next’s runtime (common issue: Next tries to write to `.next/cache` unless configured).

--- message ---
role: user
---
how to make npx -y @modelcontextprotocol/server-everything first check the local cache and download only if not available?

--- message ---
role: assistant
---
`npx` already prefers local installs and cache; it only downloads when it can’t find a suitable copy. To make that behavior reliable in Docker, you need to ensure:

1) a **persistent npm cache** directory (so it survives container restarts), and  
2) you don’t force `npx` into “always fetch latest” behavior.

### What `npx` checks (in practice)

When you run:

```sh
npx -y @modelcontextprotocol/server-everything
```

`npx` will generally resolve in this order:

1. `./node_modules/.bin` (project-local)
2. global npm bin (if configured)
3. npm cache / temporary install (download)

So the best “check local first, download only if missing” is to **make the package local** (recommended) or **persist the cache**.

---

## Option A (recommended): install it as a dependency, then run it (no download)

In your gateway project:

```sh
npm i @modelcontextprotocol/server-everything
```

Then in `mcp.json` run the local bin (no network):

```json
{
  "mcpServers": {
    "everything": {
      "command": "node",
      "args": ["./node_modules/@modelcontextprotocol/server-everything/dist/index.js"]
    }
  }
}
```

If the package exposes a bin entry, you can also do:

```json
{
  "mcpServers": {
    "everything": {
      "command": "./node_modules/.bin/server-everything",
      "args": []
    }
  }
}
```

This is deterministic and fast.

---

## Option B: keep using `npx`, but persist cache so it won’t re-download

Mount a volume for the npm cache and point `NPM_CONFIG_CACHE` to it.

### docker-compose

```yaml
services:
  mcp-server:
    environment:
      NPM_CONFIG_CACHE: /npm-cache
    volumes:
      - npm-cache:/npm-cache
      - ./mcp.json:/config/mcpServers.json:ro

volumes:
  npm-cache:
```

Now the first run downloads; subsequent restarts will typically reuse what’s in `/npm-cache`.

### Make it even more stable: pin a version
If you don’t pin, `npx` may decide it needs a newer version depending on how it resolves tags.

In `mcp.json`:

```json
{
  "mcpServers": {
    "everything": {
      "command": "npx",
      "args": ["-y", "@modelcontextprotocol/server-everything@1.2.3"]
    }
  }
}
```

(Use the actual version you want.)

---

## Option C: force “prefer offline” behavior (best-effort)

You can tell npm to prefer cached artifacts:

- `--prefer-offline` (npm flag)
- and ensure cache is persistent (Option B)

For `npx`, the most compatible way is to set npm config via env:

```yaml
environment:
  NPM_CONFIG_CACHE: /npm-cache
  NPM_CONFIG_PREFER_OFFLINE: "true"
  NPM_CONFIG_AUDIT: "false"
  NPM_CONFIG_FUND: "false"
```

This doesn’t *guarantee* zero network, but it biases toward cache.

---

## Summary

- If you want the true “check local, download only if missing”: **install the server package in your image** (Option A).
- If you want to keep `npx`: **persist npm cache** + **pin version** (Option B) and optionally set `PREFER_OFFLINE` (Option C).

If you tell me whether you want “no network at runtime” or “network allowed but only first time”, I can suggest the cleanest setup (Dockerfile + compose + `mcp.json`) for that choice.