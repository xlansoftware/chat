...
describe('FileSystemStorageService', () => {
  ...
  describe('renameNode', () => {
    it('should rename a file in the same directory', async () => {
      ...
      await storage.writeContent('/old.txt', 'content');

      const result = await storage.renameNode('/old.txt', '/new.txt');
      ...
      const oldNode = await storage.getNode('/old.txt');
      expect(oldNode).toBeNull();

      const newNode = await storage.getNode('/new.txt');
      expect(newNode?.type).toBe('file');

      const content = await storage.readContent('/new.txt');
  ...
  describe('listNodes', () => {
    ...
    it('should throw error if path points to a file', async () => {
      ...
      await expect(storage.listNodes('/test.txt')).rejects.toThrow(
        'Path points to a file, not a folder: /test.txt'
      );
    });
  });
